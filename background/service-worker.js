...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // Guard: chrome.sidePanel não existe no Opera GX (usa sidebar_action)
  if (chrome.sidePanel) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'get_formats') {
    try {
      ...
      port.onMessage.addListener((msg) => {
        // A resposta real de formatos tem status === 'formats'
        if (msg.status === 'formats') {
          responded = true;
          sendResponse(msg);
          port.disconnect();
        }
        // Repassa mensagens de inicialização (checkFirstRun) para o sidepanel
        if (msg.status === 'info' || msg.status === 'progress' || msg.status === 'success') {
          chrome.runtime.sendMessage({ action: 'companion_progress', data: msg }).catch(() => {});
        }
        // Se o companion retornar erro, repassa também
        if (msg.status === 'error') {
          responded = true;
          sendResponse({ formats: [], error: msg.error });
          port.disconnect();
        }
      });
      ...
      port.onDisconnect.addListener(() => {
        if (!responded) {
          log("get_formats: Companion desconectou antes de responder. Erro:", chrome.runtime.lastError);
          sendResponse({ formats: [], error: 'Companion disconnected' });
        }
      });
      ...
      errLog("get_formats: Erro ao conectar ao companion:", err);
      sendResponse({ formats: [], error: err.message });
    }
    return true; // async
  }
  ...
  if (request.action === 'download_youtube') {
    startCompanionDownload('download_youtube', request.url, request.cookies, request.quality, request.title, request.directDownload, request.directPath);
    sendResponse({ status: 'started_companion' });
    return true;
  }
  
  if (request.action === 'download_hls') {
    startCompanionDownload('download_hls', request.url, request.cookies, request.quality, request.title, request.directDownload, request.directPath);
    sendResponse({ status: 'started_companion' });
    return true;
  }
  
  if (request.action === 'download_html5_converted') {
    startCompanionDownload('download_html5_converted', request.url, null, request.quality, request.title, request.directDownload, request.directPath);
    sendResponse({ status: 'started_companion' });
    return true;
  }
  ...
  if (request.action === 'cancel_companion_download') {
    if (activeJob && activeJob.url === request.url && activeJob.port) {
      ...
      // onDisconnect lidará com o avanço da fila.
    } else {
  ...
  if (request.action === 'get_active_downloads') {
    const activeUrls = downloadQueue.map(j => j.url);
    if (activeJob) activeUrls.push(activeJob.url);
    sendResponse({ activeUrls });
    return true;
  }
...
function startCompanionDownload(action, url, cookies = null, quality = null, title = null, directDownload = false, directPath = '') {
  downloadQueue.push({ action, url, cookies, quality, title, directDownload, directPath });
  processDownloadQueue();
}

function executeCompanionDownload(job) {
  const { action, url, cookies, quality, title, directDownload, directPath } = job;
  ...
  try {
    ...
    activeJob.port = port;
      ...
      port.onMessage.addListener((msg) => {
        ...
        if (msg.status === 'ready_to_download') {
        ...
        } else if (msg.status === 'direct_success') {
           finishJob();
           port.disconnect();
           
           chrome.runtime.sendMessage({ action: 'download_complete', url: url }).catch(()=>{});
           ...
           chrome.storage.local.get('aster_settings', (data) => {
             ...
             if (settings.notificationsEnabled !== false) {
               chrome.notifications.create({
                 type: 'basic',
                 iconUrl: '/assets/icon.png',
                 title: t('notif_download_success_title'),
                 message: (title || 'Vídeo') + '\nSalvo em: ' + msg.filePath
               }).catch(() => {});
             }
           });
           ...
           chrome.storage.local.get('aster_history', (histData) => {
              const history = histData.aster_history || [];
              history.push({ url: url, title: title || 'Download Concluído', status: 'success', timestamp: Date.now() });
              if (history.length > 50) history.shift();
              chrome.storage.local.set({ aster_history: history });
           });
      ...
      port.postMessage({ action: action, url: url, cookies: cookies, quality: quality, directDownload: directDownload, directPath: directPath });
    } catch (err) {
